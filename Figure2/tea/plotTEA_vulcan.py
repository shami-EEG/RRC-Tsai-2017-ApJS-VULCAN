...
def plotTEA():
    ...
    # Sets the path to the TEA output file
    filename = '../run/T2500/results/T2500.tea'
    
    # Sets the path to the VULCAN output file
    vulcan_name = '../run/T2500/results/T2500.vul' 
    
    # Sets the second argument given as the species names
    species = sys.argv[1]
    
    # unpack VULCAN data into vul_data via pickle
    with open(vulcan_name, 'rb') as handle:
      vul_data = pickle.load(handle)
    vul_spec = vul_data['variable']['species'] 
    ...
    for i in np.arange(nspec):
        #plotting VULCAN outputs
        plt.loglog(vul_data['variable']['ymix'][:,vul_spec.index(spec[i])], vul_data['atm']['pco']/1e6, color=colors[color_index], lw=2, label=str(spec[i]))
